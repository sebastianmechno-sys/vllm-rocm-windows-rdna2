...
@triton.autotune(
    configs=[triton.Config({"BLOCK_N": bn}, num_warps=nw)
             for bn in (8, 16, 32, 64, 128) for nw in (1, 2, 4, 8)],
    key=["K", "N"],
)
@triton.jit
def _gemv_k_kernel(
...
def _gemv_k_sym_kernel(
    ...
    acc = tl.zeros((BLOCK_N,), dtype=c_ptr.type.element_ty)
    num_groups = K // GROUP
    for g in range(0, num_groups):
        k0 = g * GROUP
        ...
        q = (qw[:, None, :] >> shifts[None, :, None]) & 0xF
        q = tl.reshape(q, (GROUP, BLOCK_N)).to(c_ptr.type.element_ty)
        a = tl.load(a_ptr + k0 + tl.arange(0, GROUP))
        contrib = tl.sum(a[:, None] * q, axis=0)
        asum = tl.sum(a)
        s = tl.load(s_ptr + g * N + n, mask=nmask, other=0.0)
        # sum_k a*(q - BIAS)*s = s * (sum_k a*q - BIAS*sum_k a)
        acc += (contrib - BIAS * asum) * s
    tl.store(c_ptr + n, acc, mask=nmask)
    ...
    class WinRocmW4A16DequantKernel(MPLinearKernel):
        ...
        def apply_weights(self, layer, x, bias=None):
            ...
            if M == 1:
                ...
                xc = x2d.contiguous()
                if w_zp is None and _HIP_OK:
                    # Symmetric (uint4b8): native HIP dwordx4 + split-K GEMV.
                    sg = 64 if N >= 100000 else 8
                    out = torch.ops.vllm_win_hip.gemv_w4_sym(xc, w_q, w_s, G, sg, float(wb))
                elif w_zp is None:
...
def register() -> None:
    ...
    try:
        ...
        if kcls not in lst:
            lst.insert(0, kcls)
        # Symmetric (uint4b8, e.g. cyankiwi group-32): with the native HIP GEMV this beats
        # Exllama's gptq_gemm by ~6x at M=1, so place it right after the AWQ kernel. Without
        # the HIP .pyd keep it LAST (Triton fallback is slower than gptq_gemm end-to-end).
        dq = _register_dequant_kernel_class()
        if dq not in lst:
            if _HIP_OK:
                lst.insert(min(1, len(lst)), dq)
                print("vllm-win: W4A16 dequant kernel placed 2nd (native HIP sym GEMV active)")
            else:
                lst.append(dq)
                print("vllm-win: W4A16 dequant kernel placed last (Triton fallback)")
        _REGISTERED = True
